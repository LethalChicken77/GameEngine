...
Device::Device(graphics::Window &window) : window{window} {
  createInstance();
#ifdef DEBUG
  setupDebugMessenger();
#endif
...
void Device::copyImageToBuffer(VkImage image, VkBuffer buffer, uint32_t width, uint32_t height, uint32_t layerCount) 
{
    ...
    copyImageToBuffer(image, buffer, width, height, layerCount, region);
}

void Device::copyImageToBuffer(VkImage image, VkBuffer buffer, uint32_t width, uint32_t height, uint32_t layerCount, const VkBufferImageCopy &region) 
{
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
